...
export default class LabStaffDaoSQL {
  static async create(data: Record<string, any>) {
    ...
    const query = `INSERT INTO Payment (${columns}) VALUES (${placeholders})`;
  ...
  static async getByUsername(name: string) {
    const [rows] = await MySQLPool.query(
      "SELECT * FROM Payment WHERE username = ?",
      [name]
    );
    return (rows as any[])[0] || null;
  }
  ...
  static async update(_id: number, data: Record<string, any>) {
    ...
    const query = `UPDATE Payment SET ${setClause} WHERE _id = ?`;
